...
if (/1\.(0|1|2)\.(0|1|2|3|4|5)/.test($.fn.jquery) || /^1.1/.test($.fn.jquery)) {
    alert('dialogUI 需要 jQuery v1.2.6 以后版本支持!  你正使用的是 v' + $.fn.jquery);
    return;
}

// 全局函数，用来block和unbluck整个页面，弹出窗口加载一段信息或者一个层或者iframe 
$.dialogUI   = function(opts) { install(window.top, opts); };
$.undialogUI = function(opts) { remove(window.top, opts); };
...
//		CancelEvent://弹出窗口“取消”按钮响应函数，这里没设默认值，如果需要响应可直接在参数中指定
    ...
    // be default dialogUI will supress tab navigation from leaving blocking content;
...
function install(el, opts) {
    ...
    opts = $.extend({}, $.dialogUI.defaults, opts || {});
    opts.overlayCSS = $.extend({}, $.dialogUI.defaults.overlayCSS, opts.overlayCSS || {});
    var css = $.extend({}, $.dialogUI.defaults.css, opts.css || {});
	...
	if(opts.showDialog){
		...
		arr.push("           	<input id='_ButtonCancel_"+opts.id+"'  type='button' onclick=\"$.undialogUI('"+opts+"');\" value='取 消'>");
		arr.push("            </div></td></tr>");
		arr.push("      </table></td>");
	...
	if(!lyr2.length>0){
		lyr2 = full ? $('<div class="dialogUI blockOverlay" style="z-index:'+ z++ +';cursor:wait;border:none;margin:0;padding:0;width:100%;height:100%;top:0;left:0"></div>', topWin.document.body):$('<div class="dialogUI blockOverlay" style="z-index:'+ z++ +';cursor:wait;border:none;margin:0;padding:0;width:100%;height:100%;top:0;left:0"></div>');
	...
	if(!lyr3.length>0){
		lyr3 = full ? $('<div class="dialogUI blockMsg blockPage" style="z-index:'+ z +';position:fixed"></div>',topWin.document.body)
					: $('<div class="dialogUI blockMsg blockElement" style="z-index:'+ z +';display:none;position:absolute">'+opts.message+'</div>');
		lyr3.hide();
    ...
    if (ie6 || expr) {
		...
		//弹出层后把底下页面所有的下拉框隐藏
		var obj = window.index.mainFrame.I2.document.getElementsByTagName("select");
		for(kk=0;kk<obj.length;kk++)
			obj[kk].style.display='none'
		//－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－
    ...
    if(full){
		...
		if(opts.showDialog){
			...
			topWin.$.dialogUI.dragEle = lyr3;
			topWin.$.dialogUI.pointer = getPointer(event);//当前鼠标位置
			topWin.$.dialogUI.dialogPointer = {x:lyr3.css('left').substring(0,lyr3.css('left').length-2),y:lyr3.css('top').substring(0,lyr3.css('top').length-2)};//记录窗体当前位置
			...
			$(topWin.$.DialogData.iframeObj.attr('contentWindow').document).bind("mousemove",{x:"topWin.$.dialogUI.dragEle.css('left')",y:"topWin.$.dialogUI.dragEle.css('top')"}, dragMove);
...
function dragMove(event){
  ...
  if (topWin.$.dialogUI.dragEle == null ) {
		return;
	}
	...
	//鼠标的现在位置
	var x = dx  + getPointer(event).x - parseInt(topWin.$.dialogUI.pointer.x);//还需要加上iframe与div之间的表格宽度13
	var y = dy  + getPointer(event).y - parseInt(topWin.$.dialogUI.pointer.y);//还需要加上iframe与div之间的表格宽度33

	topWin.$.dialogUI.dragEle.css('left',parseInt(topWin.$.dialogUI.dialogPointer.x) + x);
	topWin.$.dialogUI.dragEle.css('top',parseInt(topWin.$.dialogUI.dialogPointer.y) + y);
}

function dragUp(){
	topWin.$.dialogUI.dragEle.unbind("mousemove", dragMove).unbind("mouseup", dragUp);
...
//    		win.$.DialogData.dialogDiv.html();
				$('#_DialogDiv_'+opts.id).fadeOut("normal");
    		bind(0, el, opts); // unbind events
    		var els = $(topWin.document.body).children().filter('.dialogUI') ;
    		if (opts.fadeOut) {
...
function bind(b, el, opts) {
    ...
    if (!b && (full && !pageBlock || !full && !$el.data('dialogUI.isBlocked'))) 
        return;
    if (!full) 
        $el.data('dialogUI.isBlocked', b);
...
function handler(e) {
    ...
    // 发生在未被block的事件是允许的
    return $(e.target).parents().children().filter('div.dialogUI').length == 0;
};
